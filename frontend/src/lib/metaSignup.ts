...
export async function launchWhatsAppEmbeddedSignup(appId: string, configId: string): Promise<WhatsAppSignupResult> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const loginConfig = {
      ...
      extras: { setup: {}, featureType: 'whatsapp_embedded_signup', sessionInfoVersion: '3', version: 'v4' },
    };
...
export function buildInstagramAuthUrl(appId: string, graphVersion: string, redirectUri: string): string {
  const params = new URLSearchParams({
    ...
    response_type: 'code',
    scope: 'pages_show_list,pages_read_engagement,instagram_basic,instagram_business_manage_messages,business_management,pages_messaging',
  });
