      ...
      {open && debounced.trim().length > 0 && (
        <div className="absolute z-50 mt-2 w-full overflow-hidden rounded-lg border border-white/10 bg-[#15171d] shadow-xl">
          {isFetching && !data ? (
            ...
            <p className="px-4 py-3 text-sm text-white/40">No results for "{debounced}"</p>
          ) : (
...
function ResultGroup({ icon: Icon, label, children }: { icon: typeof Building2; label: string; children: ReactNode }) {
  return (
    <div>
      <p className="flex items-center gap-1.5 px-4 pt-2.5 pb-1 text-[11px] font-semibold uppercase tracking-wide text-white/30">
